...
- 결과: 합성 관절 좌표를 사용한 순수 Swift 특징 계산을 #6에서 검증했다. 손바닥 너비·길이·기하평균, 손가락 직진도·굽힘 각도·손끝 거리, 엄지–검지 거리와 양손 관계가 평행이동·회전·균일 스케일 뒤에도 재현되는 것을 단위 테스트로 확인했다. 실제 Joint 좌표 비교와 최종 정규화 방식·임계값 결정은 미실행이다.
...
| --- | --- | --- | --- | --- |
| 2026-08-08 | Pose 특징 계산 Spike | macOS 26.5.2, Xcode 26.6, Swift 6.3.3, 합성 좌표 | 순수 Swift XCTest 8개 통과. 실기기·실제 ARKit 좌표는 미검증 | #6 |
