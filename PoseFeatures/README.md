...
`PoseFeatureVerificationRecord` currently uses schema version `1`. Readers
must reject records with an unsupported future schema version. A breaking
record change must increment the schema version and document how version `1`
records are interpreted or migrated.
