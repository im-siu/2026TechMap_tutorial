...
Issue 제목의 태그와 본문을 보고 현재 목적을 먼저 판단한다.

### `[DISCOVERY]`

- 코드를 바로 만들지 않는다.
- 대상 독자, 문제, 성공 기준, 대안과 불확실성을 구조화한다.
- 하나의 결론만 제시하지 말고 의미 있는 선택지를 비교한다.
- 확인된 사실과 가설을 분리하고 조사할 공식 자료를 연결한다.
...
- 재사용 가능한 완성 구조보다 한 가지 질문에 답하는 최소 실험을 우선한다.
- 성공뿐 아니라 실패 조건, 환경과 관찰 결과를 남긴다.
- Spike 코드를 본 구현으로 채택하려면 별도 `[TUTORIAL]` 또는 `[DECISION]` 검토를 거친다.

### `[TUTORIAL]`, `[VERIFY]`, `[FIX]`

- Issue 범위와 완료 조건을 기준으로 구현하거나 검증한다.
- 코드, DocC와 코드 리스팅의 일치를 확인한다.
- 실제 수행하지 않은 검증을 완료로 표시하지 않는다.
...
- Issue 하나의 목적에 집중한다.
- 브랜치와 커밋 규칙은 Wiki의 [Issue and PR Conventions](https://github.com/im-siu/2026TechMap_tutorial/wiki/Issue-and-PR-Conventions)를 따른다.
- 요청받지 않은 커밋, push, merge 또는 PR 생성은 하지 않는다.
- PR 설명에는 `Closes #번호`, AI 활용 범위, 검증 결과와 미검증 항목을 남긴다.
- 중요한 설계 선택은 코드만으로 암묵적으로 결정하지 말고 `[DECISION]` Issue의 합의를 요구한다.
